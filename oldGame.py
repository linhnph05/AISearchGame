...
class RushHourGame:
    ...
    def loadAssets(self):
        ...
        # Load target car image
        try:
            image = pygame.image.load("Resource/car1.png")
            rotatedImage = pygame.transform.rotate(image, 90)
            self.targetCarImage = rotatedImage
        except pygame.error:
            print("Warning: car1.png not found, using default car appearance")
    ...
    def playGame(self):
        if self.gameState == GameState.STOPPED:
            ...
            if self.currentAlgorithm == Algorithm.A_STAR:
                ...
                self.runIds()  # Now using the properly named runIds method
            else:
                # For now, use A* for other algorithms (you can implement UCS later)
                self.runAStar()
    ...
    def runAStar(self):
        ...
        try:
            moves = self.board.aStar(self.initialState)
            endTime = time.time()
    ...
    def drawCars(self):
        ...
        for car in self.cars:
            ...
            pygame.draw.rect(self.screen, color, carRect, border_radius=6)
            
            # Draw car details (number)
            number = str(car.carId) if not car.isTarget else ""
            if car.isTarget:
                # Use car image if available, otherwise draw simple car icon
                if hasattr(self, 'targetCarImage') and self.targetCarImage:
                    # Scale the car image to fit the car rectangle
                    scaledCarImage = pygame.transform.scale(self.targetCarImage, (width+10, height+10))
                    
                    # # Rotate the image if the car is vertical
                    # if car.orientation == 'V':
                    #     scaledCarImage = pygame.transform.rotate(scaledCarImage, 90)
                    
                    # Draw the car image
                    self.screen.blit(scaledCarImage, carRect)
                else:
                    # Fallback to drawing simple car icon
                    carWidth = width - 10
                    carHeight = height - 12
                    ...
                    if car.orientation == 'H':
                        # Wheels
                        wheelRadius = height * 0.15
                        wheelY = carRect.centery + height * 0.1
                        frontWheelX = carRect.left + carWidth - wheelRadius - 2
                        backWheelX = carRect.left + wheelRadius + 8
                        
                        pygame.draw.circle(self.screen, Colors.WHITE, 
                                          (frontWheelX, wheelY), wheelRadius, 2)
                        pygame.draw.circle(self.screen, Colors.WHITE, 
                                          (backWheelX, wheelY), wheelRadius, 2)
                    else:
                        # For vertical car, draw horizontal lines
                        line1Y = carRect.top + height * 0.25
                        line2Y = carRect.top + height * 0.75
                        pygame.draw.line(self.screen, Colors.WHITE, 
                                        (carRect.left + 5, line1Y), 
                                        (carRect.right - 5, line1Y), 2)
                        pygame.draw.line(self.screen, Colors.WHITE, 
                                        (carRect.left + 5, line2Y), 
                                        (carRect.right - 5, line2Y), 2)
            else:
                # Draw number for other cars
